...
#include "kswDequeIterator.h"
#include <algorithm>

namespace KSW{
	template<class T, class Alloc = kswAllocator<T>, size_t BufSiz = 0>
	class kswDeque{
...
#pragma region typedef和成员变量的定义
	public:
		...
		typedef typename __deque_iterator<T, T&, T*, BufSiz>::iterator iterator; // 迭代器，#include "kswDequeIterator.h"
...
#pragma region kswDeque的初始化

	protected:
		typedef simple_alloc<value_type, Alloc>		data_allocator; // kswDeque节点的空间配置器
		typedef simple_alloc<pointer, Alloc>		map_allocator; // kswDeque管控中心（map）的空间配置器
	public:
		/*
		构造函数
		*/
		kswDeque(int n, const value_type& value) :start(), finish(), map(0), map_size(0)
		{
		...
		默认构造函数
		*/
		kswDeque() :start(), finish(), map(0), map_size(0) { fill_initialize(0, 0); }
	protected:
		...
		*/
		void create_map_and_nodes(size_type num_elements)
		{
			...
			// 为管控中心（map）的每个现用节点配置缓冲区，所有缓冲区加起来就是kswDeque的可用空间
			map_pointer cur;
...
#pragma region kswDeque的查询操作

	public:
		iterator begin(){ return start; } // 获得kswDeque的头部

		iterator end(){ return finish; } // 获得kswDeque的尾部

		reference operator[](size_type n){ return start[difference_type(n)]; } // 获得kswDeque第n个元素

		reference front(){ return *start; } // 获得kswDeque的头部的值

		/*
		因为缓冲区是前闭后开的区间，获得kswDeque的尾部时需要finish回退一个步长
		*/
		reference back()
		{
			iterator tmp = finish;
		...
		*/
		...
		size_type max_size() const{ return finish - start; } // 获得kswDeque的最大程长度

		bool empty()const{ return finish == start; } // kswDeque是否为空

#pragma endregion

#pragma region kswDeque元素的添加和删除操作
